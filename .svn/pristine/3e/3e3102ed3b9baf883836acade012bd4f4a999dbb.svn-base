...
public class WeightTrackingFragment extends Fragment {
    ...
    private int user_id;
    private ArrayList<String> measurements_size = new ArrayList<>();

    private ArrayList<String> measurements_date = new ArrayList<>();
    private ArrayList<String> measurements_Id = new ArrayList<>();
    public WeightTrackingFragment() {
        // Required empty public constructor
    }

    private void apiCall(int id, String measurement_type, String name) {
        ...
        //reading the response from the server call
    ...
    //this method calculates the users BMI using their weight and height
    public double bmi_calc(String weight, String height) {
        double height_meters = Double.parseDouble(height) / 100;

        return Math.round(Integer.parseInt(weight) / Math.pow(height_meters, 2));


    }

    public double ffmi_calc(String height, String weight, String fat_percentage) {
        height_feet = Double.parseDouble(height) / 30.38;
        ...
        double height_ft = Math.floor(Double.parseDouble(height) / 30.38);
        double height_in = Math.round(height_inch);
        double lean_muscle = Double.parseDouble(weight) * (1.0 - (Double.parseDouble(fat_percentage) / 100.0));

        DecimalFormat value = new DecimalFormat("#.#");

        String ffmi = value.format((lean_muscle / 2.2) / Math.pow(((height_ft * 12.0 + height_in) * 0.0254), 2) * 2.20462);
        return Double.parseDouble(ffmi);
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ImageButton bmi_helpBtn = view.findViewById(R.id.imcHelp);
        ImageButton ffmi_helpBtn = view.findViewById(R.id.ffmiHelp);
        ...
        bmi_helpBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new SweetAlertDialog(getContext(), SweetAlertDialog.CUSTOM_IMAGE_TYPE)
                        .setTitleText(R.string.imclabel)
                        .setContentText(getText(R.string.BMI_description).toString())
        ...
        ffmi_helpBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                new SweetAlertDialog(getContext(), SweetAlertDialog.CUSTOM_IMAGE_TYPE)
                        .setTitleText(R.string.ffmilabel)

                        .setContentText(getText(R.string.ffmi_description).toString())
                        .setCustomImage(R.drawable.ic_info_black_24dp)
                        .show();
            }
        });

        //retrieving the users recent measurements from the database
...
//                    prefConFig.distplayToast("im here");
                    ...
                    if (weight != null && fat_percentage != null && !prefConFig.readHeight().equals("no height found")) {
                        ...
                        double ffmi = ffmi_calc(prefConFig.readHeight(), prefConFig.readWeight(), fat_percentage);
...
//                        height_meters = Double.parseDouble(prefConFig.readHeight()) / 100;
                        double bmi = bmi_calc(prefConFig.readWeight(), prefConFig.readHeight());
//                                Math.round(Integer.parseInt(prefConFig.readWeight()) / Math.pow(height_meters, 2));
...
//weight image button
        ImageButton weight_imageButton = view.findViewById(R.id.weightDetailsButton);
...
//fat image button
        ImageButton fat_imgbtn = view.findViewById(R.id.fat_imgbtn);
        ...
        //water image button
        ImageButton water_imgbtn = view.findViewById(R.id.water_imgBtn);
        ...
        ImageButton muscle_imgbtn = view.findViewById(R.id.muscles_imgBtn);
